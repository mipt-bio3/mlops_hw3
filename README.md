...
**Сборка:**  
— устанавливаются Python-зависимости и генерируется модель
— компилируются gRPC-спецификации из `.proto`-файлов  
— собираются два Docker-образа: `blue` (версия модели v1.0.0) и `green` (версия модели v1.1.0)

**Деплой:**  
— запускается локальный стек через Docker Compose с Nginx в качестве reverse-прокси  
— сначала активен `blue`-сервис, затем происходит переключение на `green`, и в завершение — откат обратно на `blue`
